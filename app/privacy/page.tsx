...
const sections: LegalSection[] = [
  ...
  },
  {
    id: 'collection',
    title: '我们处理的信息',
    content: <><p>我们仅在提供和保护本服务所需的范围内处理以下信息：</p><ul><li><strong>账户信息：</strong>通过 GitHub OAuth 获得的账户标识、用户名或显示名称、邮箱地址、头像，以及登录会话所需的数据。</li><li><strong>设备与安全信息：</strong>登录时间、会话有效期、IP 地址、浏览器或 User-Agent，以及防止滥用和排查故障所需的安全记录。</li><li><strong>服务与用量信息：</strong>所属用户组、API 调用计数、限速窗口、日/周/月用量、Credits 余额和权益有效期。除非具体 API 另有说明，我们不会为用量统计保存你的 API 请求正文。</li><li><strong>兑换与订单信息：</strong>兑换码状态、权益发放记录，以及爱发电提供的用户标识、订单号、方案或 SKU、金额、购买周期和订单状态。爱发电订单回调在特定商品场景下还可能包含留言、联系方式或收货信息。</li><li><strong>分析信息：</strong>页面访问和基础性能数据，用于了解服务是否正常运行和改进界面。</li></ul></>,
  },
  {
    id: 'purpose',
    title: '处理目的与依据',
    content: <ul><li>创建账户、完成身份验证并维持登录状态。</li><li>执行 API 限速、用量统计、用户组与 Credits 权益。</li><li>验证兑换码和爱发电订单，发放、续期或恢复已购买权益。</li><li>防止欺诈、共享凭据、自动化滥用和其他危害服务安全的行为。</li><li>排查故障、衡量服务质量、回应咨询并履行适用的法律义务。</li></ul>,
  },
  {
    id: 'oauth',
    title: '第三方登录与爱发电绑定',
    content: <><p>GitHub OAuth 用于登录。本服务不会因登录而读取你的私有仓库内容。你应同时查阅 GitHub 的隐私规则。</p><p>当爱发电 OAuth2 绑定功能上线并由你主动授权后，我们将保存授权返回的爱发电用户标识，用于把支付订单与 AccessHub 账户关联。OAuth 客户端密钥仅保存在服务端，不会发送到浏览器。你可以联系我们解除绑定；解除绑定不会自动删除为履行订单或防止重复发放而必须保留的记录。</p></>,
  },
  {
    id: 'sharing',
    title: '委托处理、共享与跨境',
