...
const viewSlugs: Record<View, string> = { '概览': 'overview', '兑换码': 'redeem', '用户组': 'groups', '爱发电': 'afdian', 'API 文档': 'api' }
const slugViews = Object.fromEntries(Object.entries(viewSlugs).map(([view, slug]) => [slug, view])) as Record<string, View>
const adminViews: View[] = ['用户组', '爱发电']
const viewMeta: Record<View, { eyebrow: string; description: string }> = {
  ...
  '爱发电': { eyebrow: '管理员', description: '维护爱发电方案与用户组或 credits 之间的自动到账规则。' },
...
export default function Page() {
  ...
  useEffect(() => {
    const syncView = () => setActive(slugViews[new URL(window.location.href).searchParams.get('view') || ''] || '概览')
    syncView()
  ...
  const navigate = (view: View, replace = false) => {
    ...
    if (view === '概览') url.searchParams.delete('view')
    else url.searchParams.set('view', viewSlugs[view])
    window.history[replace ? 'replaceState' : 'pushState']({}, '', url)
  }

  useEffect(() => {
    if (!loading && dashboard && !isAdmin && adminViews.includes(active)) navigate('概览', true)
  // `navigate` only derives browser state and does not need a stable identity.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [active, dashboard, isAdmin, loading])

  useEffect(() => { document.title = `${active} · AccessHub` }, [active])
...
function NavButton({ view, active, onClick }: { view: View; active: boolean; onClick: () => void }) {
  ...
  return <button onClick={onClick} aria-current={active ? 'page' : undefined} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm transition focus-visible:ring-2 focus-visible:ring-[#7893ee] active:translate-y-px ${active ? 'bg-[#edf2ff] font-medium text-[#3157d5]' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700'}`}><Icon size={17}/>{view}</button>
}
