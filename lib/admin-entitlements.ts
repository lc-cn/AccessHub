...
export function parseSubscriptionPlanPolicyInput(body: Record<string, unknown>, rateLimitFallback = -1): ParseResult<SubscriptionPlanPolicyInput> {
...
export function parseBenefitInput(body: Record<string, unknown>, durationFallback?: { value: number; unit: EntitlementUnit }): ParseResult<BenefitInput> {
  const kind: RedeemKind = body.kind === 'credits' ? 'credits' : 'plan'
  const planId = kind === 'plan' ? String(body.planId || '').trim() : null
  ...
  if (kind === 'plan' && !planId) return { ok: false, error: '请选择订阅计划' }
  return { ok: true, value: { kind, planId, credits, ...duration } }
}
