...
export function exceededAllowance(policy: AllowancePolicy, usage: UsageSnapshot) {
  const checks = [
  ...
  return checks.find((item) => !isUnlimited(item.limit) && item.used >= item.limit!) ?? null
}

export function selectAllowance<T extends { policy: AllowancePolicy; usage: UsageSnapshot }>(candidates: T[]) {
  let exceeded: ReturnType<typeof exceededAllowance> = null
  for (const candidate of candidates) {
    const candidateExceeded = exceededAllowance(candidate.policy, candidate.usage)
