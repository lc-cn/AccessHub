...
export function AccountShell({ user, children }: { user: { name: string; email: string; image?: string | null }; children: React.ReactNode }) {
  ...
  return <div className="min-h-dvh bg-[#f3f6fb] text-[#172033]">
    ...
    <aside className="fixed inset-y-0 left-0 z-20 hidden w-64 border-r border-[#e3e9f3] bg-white px-5 py-6 lg:flex lg:flex-col">
      ...
      <nav className="mt-4 flex-1 space-y-1" aria-label="个人中心导航">{items.map((item) => { const Icon = item.icon; const active = pathname === item.href; return <Link key={item.href} href={item.href} aria-current={active ? 'page' : undefined} className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm transition ${active ? 'bg-[#edf2ff] font-medium text-[#3157d5]' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700'}`}><Icon size={17}/>{item.label}</Link> })}</nav>
      <div className="rounded-[18px] bg-[#f4f6fa] p-3.5"><div className="flex items-center gap-3"><Avatar user={user}/><div className="min-w-0"><p className="truncate text-sm font-medium">{user.name}</p><p className="mt-0.5 truncate text-[11px] text-slate-400">{user.email}</p></div><button onClick={() => void signOut()} aria-label="退出登录" className="ml-auto rounded-lg p-2 text-slate-400 transition hover:bg-white hover:text-slate-600"><LogOut size={15}/></button></div></div>
    </aside>
    <main id="account-content" className="min-h-dvh lg:pl-64"><header className="border-b border-[#e3e9f3] bg-white"><div className="mx-auto flex min-h-[76px] max-w-6xl items-center justify-between gap-4 px-5 py-4 sm:px-8"><div><p className="text-[11px] font-medium tracking-[.12em] text-slate-400">个人中心</p><h1 className="mt-1 text-xl font-semibold tracking-tight">管理你的账户</h1></div><div className="flex items-center gap-3 lg:hidden"><Link href="/dashboard" aria-label="返回控制台" className="grid size-9 place-items-center rounded-xl border border-slate-200 text-slate-500"><ArrowLeft size={16}/></Link><Avatar user={user}/></div></div><nav className="flex gap-1 overflow-x-auto px-5 pb-3 lg:hidden" aria-label="个人中心导航">{items.map((item) => <Link key={item.href} href={item.href} className={`shrink-0 rounded-lg px-3 py-1.5 text-xs font-medium ${pathname === item.href ? 'bg-[#182238] text-white' : 'text-slate-500'}`}>{item.label}</Link>)}</nav></header><div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 lg:py-10">{children}</div></main>
  </div>
}

function Avatar({ user }: { user: { name: string; image?: string | null } }) { return <span className="grid size-9 shrink-0 place-items-center overflow-hidden rounded-xl bg-[#e7ebf3] text-xs font-semibold text-slate-600">{user.image ? <img src={user.image} alt={`${user.name} 的头像`} className="size-full object-cover"/> : user.name.slice(0, 1).toUpperCase()}</span> }
