...
function Status({ value }: { value: string }) { const good = ['active', 'succeeded', 'processed', 'admin'].includes(value); const bad = ['failed', 'past_due', 'canceled', 'expired'].includes(value); return <span className={`rounded-md px-2 py-0.5 text-[10px] ${good ? 'bg-emerald-50 text-emerald-700' : bad ? 'bg-rose-50 text-rose-600' : 'bg-amber-50 text-amber-700'}`}>{value}</span> }
