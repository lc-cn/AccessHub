...
export function CommerceResources({ resource }: { resource: Resource }) {
  ...
  return <section><div className="mb-5"><span className="grid size-10 place-items-center rounded-xl bg-[#edf2ff] text-[#3157d5]"><Icon size={18}/></span><h2 className="mt-5 text-2xl font-semibold">{config.title}</h2><p className="mt-2 text-sm text-slate-500">{config.description}</p></div><label className="mb-4 flex h-11 items-center gap-2 rounded-xl bg-white px-3 text-slate-400"><Search size={15}/><input value={query} onChange={(event) => setQuery(event.target.value)} className="w-full bg-transparent text-sm outline-none" placeholder={`搜索${config.title}`}/></label>{error && <p className="mb-4 rounded-xl bg-rose-50 px-3 py-2 text-xs text-rose-600">{error}</p>}{loading ? <div className="h-48 animate-pulse rounded-[20px] bg-white"/> : rows.length === 0 ? <div className="rounded-[20px] bg-white p-12 text-center text-sm text-slate-400">暂无记录</div> : <div className="space-y-2">{rows.map((row) => <ResourceRow key={row.id} resource={resource} row={row} updating={updatingId === row.id} onTransition={transition}/>)}</div>}</section>
}
...
function SubscriptionRow({ item, updating, onTransition }: { item: Subscription; updating: boolean; onTransition: (id: string, status: Subscription['status']) => Promise<void> }) {
  ...
  return <article className="rounded-[16px] bg-white px-5 py-4"><div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div className="min-w-0"><div className="flex items-center gap-2"><h3 className="truncate text-sm font-medium">{item.userName || item.userId || '等待用户激活'}</h3><Status value={item.status}/></div><code className="mt-1 block truncate text-[10px] text-slate-400">{item.id}</code><p className="mt-2 truncate text-xs text-slate-500">{item.planName}{item.skuCode ? ` · ${item.skuCode}` : ''}</p></div><time className="text-[11px] text-slate-400">{item.currentPeriodEnd ? `当前周期至 ${formatDate(item.currentPeriodEnd)}` : '尚未开始计费周期'}</time></div>{options.length > 0 && <div className="mt-4 flex justify-end gap-2 border-t border-slate-100 pt-3"><select value={next} onChange={(event) => setNext(event.target.value as Subscription['status'] | '')} className="h-9 rounded-lg border border-slate-200 bg-white px-2 text-xs outline-none"><option value="">选择状态操作</option>{options.map((status) => <option key={status} value={status}>{status}</option>)}</select><button disabled={!next || updating} onClick={() => next && void onTransition(item.id, next)} className="h-9 rounded-lg bg-[#182238] px-3 text-xs text-white disabled:opacity-40">{updating ? '处理中…' : '确认变更'}</button></div>}</article>
}
...
function Status({ value }: { value: string }) { const good = ['active', 'succeeded', 'processed', 'ignored', 'admin'].includes(value); const bad = ['failed', 'past_due', 'canceled', 'expired'].includes(value); return <span className={`rounded-md px-2 py-0.5 text-[10px] ${good ? 'bg-emerald-50 text-emerald-700' : bad ? 'bg-rose-50 text-rose-600' : 'bg-amber-50 text-amber-700'}`}>{value}</span> }
