...
export type ApiService = { id: string; code: string; name: string; description: string; baseUrl: string; authType: 'none' | 'bearer' | 'header' | 'query' | 'basic'; authConfigured: boolean; enabled: boolean; createdAt: string; updatedAt: string; apis: ServiceApi[] }

export type AdminData = { plans: AdminPlan[]; codes: RedeemCode[]; afdianMappings: AfdianMapping[]; orders: Order[]; skus: Sku[]; users: AdminUser[]; logs: ActivityLog[]; subscriptions: Subscription[]; payments: Payment[]; providerEvents: ProviderEvent[]; services: ApiService[]; afadianWebhookConfigured: boolean; afadianMessengerConfigured?: boolean }
