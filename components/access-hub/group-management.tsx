...
type Props = { groups: DashboardGroup[]; mode: 'list' | 'new' | 'edit'; groupId?: string; onChanged: () => Promise<void> }
...
function formFor(group: AdminGroup): GroupForm { return { name: group.name, description: group.description, rateLimit: String(group.rateLimit), dailyLimit: String(group.dailyLimit ?? -1), weeklyLimit: String(group.weeklyLimit ?? -1), monthlyLimit: String(group.monthlyLimit ?? -1), isDefault: group.isDefault } }

export function GroupManagement({ groups, mode, groupId, onChanged }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    } catch (error) { setNotice({ tone: 'error', text: error instanceof Error ? error.message : '用户组读取失败' }) }
    finally { setLoading(false) }
  }, [])

  useEffect(() => { void load() }, [load])
  const selected = useMemo(() => adminGroups.find((group) => group.id === groupId) ?? null, [adminGroups, groupId])
  useEffect(() => {
  ...
  const submit = async () => {
    ...
    setNotice(null)
    try {
      const result = await requestJson<{ group: AdminGroup }>('/api/admin', { method: mode === 'new' ? 'POST' : 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ type: 'group', groupId, ...form }) })
      await Promise.all([load(), onChanged()])
      if (mode === 'new') router.replace(`/admin/groups/${result.group.id}`)
      else { setForm(formFor(result.group)); setNotice({ tone: 'success', text: '策略已保存' }) }
    } catch (error) { setNotice({ tone: 'error', text: error instanceof Error ? error.message : '保存失败' }) }
    finally { setSaving(false) }
  }

  if (mode === 'list') return <section>
    <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-medium tracking-wide text-[#3157d5]">访问策略</p><h2 className="mt-1 text-2xl font-semibold tracking-tight">用户组</h2><p className="mt-2 text-sm text-slate-500">每个策略单独维护，修改时进入对应详情页。</p></div><Link href="/admin/groups/new" className="flex items-center justify-center gap-2 rounded-xl bg-[#3157d5] px-4 py-2.5 text-sm font-medium text-white transition hover:bg-[#284bc2] active:translate-y-px"><Plus size={16}/>新建用户组</Link></div>
    ...
    {loading ? <GroupSkeleton/> : adminGroups.length === 0 ? <Link href="/admin/groups/new" className="block w-full rounded-[22px] border border-dashed border-slate-300 bg-white p-12 text-center text-sm text-slate-500">还没有用户组，创建第一个用户组</Link> : visibleGroups.length === 0 ? <p className="rounded-[18px] bg-white px-5 py-12 text-center text-sm text-slate-400">没有匹配的用户组</p> : <div className="space-y-3">{visibleGroups.map((group) => {
      const memberCount = groups.find((item) => item.id === group.id)?.memberCount ?? 0
      return <Link key={group.id} href={`/admin/groups/${group.id}`} className="grid w-full grid-cols-[1fr_auto] gap-5 rounded-[18px] bg-white p-5 text-left transition hover:-translate-y-0.5 hover:shadow-[0_14px_34px_rgba(39,55,92,.07)] active:translate-y-px"><div className="min-w-0"><div className="flex items-center gap-2"><h3 className="truncate font-semibold">{group.name}</h3>{group.isDefault && <span className="rounded-md bg-[#edf2ff] px-2 py-0.5 text-[10px] font-medium text-[#3157d5]">默认</span>}</div><p className="mt-1 truncate text-xs text-slate-400">{group.description || '暂无描述'}</p><div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-xs text-slate-500"><span className="flex items-center gap-1.5"><Gauge size={13}/>{quotaText(group.rateLimit)} 次/分钟</span><span>{quotaText(group.dailyLimit)} / 日</span><span>{quotaText(group.weeklyLimit)} / 周</span><span>{quotaText(group.monthlyLimit)} / 月</span></div></div><div className="text-right"><p className="text-2xl font-semibold tabular-nums">{memberCount.toLocaleString()}</p><p className="mt-1 text-[11px] text-slate-400">有效成员</p></div></Link>
    })}</div>}
  </section>

  if (loading || (mode === 'edit' && !selected && !notice)) return <EditorSkeleton/>
  if (mode === 'edit' && !selected) return <EmptyEditor message="这个用户组不存在或已被删除"/>
  const memberCount = groups.find((group) => group.id === groupId)?.memberCount ?? 0

  return <section className="mx-auto max-w-3xl">
    <Link href="/admin/groups" className="mb-5 inline-flex items-center gap-2 text-sm text-slate-500 transition hover:text-[#3157d5]"><ArrowLeft size={15}/>返回用户组</Link>
    <form onSubmit={(event) => { event.preventDefault(); void submit() }} className="rounded-[24px] bg-white p-6 shadow-[0_18px_55px_rgba(39,55,92,.06)] sm:p-8">
      <div className="flex items-start justify-between"><div><div className="flex items-center gap-2"><p className="text-xs text-slate-400">{mode === 'new' ? '新策略' : '策略详情'}</p>{dirty && <span className="rounded bg-amber-50 px-1.5 py-0.5 text-[10px] text-amber-700">未保存</span>}</div><h2 className="mt-1 text-2xl font-semibold">{mode === 'new' ? '创建用户组' : selected?.name}</h2></div>{mode === 'edit' && <div className="text-right"><p className="text-xl font-semibold tabular-nums">{memberCount}</p><p className="text-[10px] text-slate-400">有效成员</p></div>}</div>
      <div className="mt-8 space-y-5"><Field label="用户组名称"><input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} maxLength={80} className="access-input" placeholder="例如：专业用户"/></Field><Field label="说明"><textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} maxLength={240} rows={3} className="access-input resize-none" placeholder="说明适用对象和使用场景"/></Field><div className="grid gap-3 sm:grid-cols-2"><Field label="每分钟请求"><QuotaInput value={form.rateLimit} onChange={(value) => setForm({ ...form, rateLimit: value })}/></Field><Field label="每日请求"><QuotaInput value={form.dailyLimit} onChange={(value) => setForm({ ...form, dailyLimit: value })}/></Field><Field label="每周请求"><QuotaInput value={form.weeklyLimit} onChange={(value) => setForm({ ...form, weeklyLimit: value })}/></Field><Field label="每月请求"><QuotaInput value={form.monthlyLimit} onChange={(value) => setForm({ ...form, monthlyLimit: value })}/></Field></div><p className="text-[11px] text-slate-400">填写 -1 表示对应周期无限制。</p><label className={`flex items-center justify-between rounded-xl border px-4 py-3 ${form.isDefault ? 'border-[#b9c9ff] bg-[#f3f6ff]' : 'border-slate-200'}`}><span><span className="flex items-center gap-2 text-sm font-medium"><Star size={15} className={form.isDefault ? 'fill-[#3157d5] text-[#3157d5]' : 'text-slate-400'}/>默认用户组</span><span className="mt-1 block text-xs text-slate-400">未配置其他权益的用户自动归入此组</span></span><input type="checkbox" checked={form.isDefault} disabled={selected?.isDefault} onChange={(event) => setForm({ ...form, isDefault: event.target.checked })} className="size-4 accent-[#3157d5]"/></label>{notice && <p className={`flex items-center gap-2 rounded-xl px-3 py-2.5 text-xs ${notice.tone === 'success' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-600'}`}>{notice.tone === 'success' && <CheckCircle2 size={14}/>} {notice.text}</p>}</div>
      <div className="mt-7 flex gap-2 border-t border-slate-100 pt-5">{dirty && <button type="button" onClick={() => baseline && setForm(baseline)} className="flex items-center gap-1.5 rounded-xl px-3 py-2.5 text-sm text-slate-500 hover:bg-slate-50"><RotateCcw size={14}/>重置</button>}<button disabled={saving || !dirty} className="ml-auto flex items-center gap-2 rounded-xl bg-[#182238] px-5 py-2.5 text-sm font-medium text-white hover:bg-slate-800 disabled:opacity-40"><Save size={15}/>{saving ? '保存中…' : mode === 'new' ? '创建用户组' : '保存策略'}</button></div>
    </form>
  </section>
}
...
function EmptyEditor({ message }: { message: string }) { return <div className="mx-auto max-w-2xl rounded-[22px] bg-white p-10 text-center"><p className="text-sm text-slate-500">{message}</p><Link href="/admin/groups" className="mt-5 inline-flex text-sm font-medium text-[#3157d5]">返回用户组列表</Link></div> }
