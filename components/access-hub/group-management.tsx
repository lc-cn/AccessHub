...
export function GroupManagement({ groups, onChanged }: { groups: DashboardGroup[]; onChanged: () => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    else if (selected) setForm({ name: selected.name, description: selected.description, rateLimit: String(selected.rateLimit), dailyLimit: selected.dailyLimit == null ? '' : String(selected.dailyLimit), weeklyLimit: selected.weeklyLimit == null ? '' : String(selected.weeklyLimit), monthlyLimit: selected.monthlyLimit == null ? '' : String(selected.monthlyLimit), isDefault: selected.isDefault })
  }, [creating, selected])
  ...
  return <div className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(360px,.9fr)]">
    <section>
      ...
      {loading ? <GroupSkeleton/> : adminGroups.length === 0 ? <button onClick={() => setCreating(true)} className="w-full rounded-[22px] border border-dashed border-slate-300 bg-white p-12 text-center text-sm text-slate-500">还没有用户组，创建第一个用户组</button> : <div className="space-y-3">{adminGroups.map((group) => {
        ...
        return <button key={group.id} onClick={() => { setCreating(false); setSelectedId(group.id); setNotice(null) }} className={`grid w-full grid-cols-[1fr_auto] gap-5 rounded-[18px] p-5 text-left transition active:translate-y-px ${active ? 'bg-[#182238] text-white shadow-[0_18px_42px_rgba(24,34,56,.16)]' : 'bg-white hover:-translate-y-0.5 hover:shadow-[0_14px_34px_rgba(39,55,92,.07)]'}`}><div className="min-w-0"><div className="flex items-center gap-2"><h3 className="truncate font-semibold">{group.name}</h3>{group.isDefault && <span className={`rounded-md px-2 py-0.5 text-[10px] font-medium ${active ? 'bg-white/10 text-slate-200' : 'bg-[#edf2ff] text-[#3157d5]'}`}>默认</span>}</div><p className="mt-1 truncate text-xs text-slate-400">{group.description || '暂无描述'}</p><div className={`mt-4 flex flex-wrap gap-x-5 gap-y-2 text-xs ${active ? 'text-slate-300' : 'text-slate-500'}`}><span className="flex items-center gap-1.5"><Gauge size={13}/>{group.rateLimit.toLocaleString()} 次/分钟</span><span>{group.dailyLimit?.toLocaleString() ?? '不限额'} / 日</span><span>{group.weeklyLimit?.toLocaleString() ?? '不限额'} / 周</span><span>{group.monthlyLimit?.toLocaleString() ?? '不限额'} / 月</span></div></div><div className="text-right"><p className="text-2xl font-semibold tabular-nums">{memberCount.toLocaleString()}</p><p className="mt-1 text-[11px] text-slate-400">有效成员</p></div></button>
      })}</div>}
    </section>
    ...
    <aside className="h-fit rounded-[22px] bg-white p-6 shadow-[0_18px_55px_rgba(39,55,92,.06)] xl:sticky xl:top-24">
      ...
      {(creating || selected) ? <form onSubmit={(event) => { event.preventDefault(); void submit() }} className="mt-6 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3"><Field label="每分钟请求"><input type="number" min="1" value={form.rateLimit} onChange={(event) => setForm({ ...form, rateLimit: event.target.value })} className="access-input tabular-nums"/></Field><Field label="每日请求"><input type="number" min="1" value={form.dailyLimit} onChange={(event) => setForm({ ...form, dailyLimit: event.target.value })} className="access-input tabular-nums" placeholder="留空则不限"/></Field><Field label="每周请求"><input type="number" min="1" value={form.weeklyLimit} onChange={(event) => setForm({ ...form, weeklyLimit: event.target.value })} className="access-input tabular-nums" placeholder="留空则不限"/></Field><Field label="每月请求"><input type="number" min="1" value={form.monthlyLimit} onChange={(event) => setForm({ ...form, monthlyLimit: event.target.value })} className="access-input tabular-nums" placeholder="留空则不限"/></Field></div>
