...
type PlanForm = { name: string; description: string; rateLimit: string; dailyLimit: string; weeklyLimit: string; monthlyLimit: string; isDefault: boolean }
type Props = { plans: DashboardPlan[]; mode: 'list' | 'new' | 'edit'; planId?: string; onChanged: () => Promise<void> }
const emptyForm: PlanForm = { name: '', description: '', rateLimit: '60', dailyLimit: '10000', weeklyLimit: '50000', monthlyLimit: '150000', isDefault: false }
function formFor(plan: AdminPlan): PlanForm { return { name: plan.name, description: plan.description, rateLimit: String(plan.rateLimit), dailyLimit: String(plan.dailyLimit ?? -1), weeklyLimit: String(plan.weeklyLimit ?? -1), monthlyLimit: String(plan.monthlyLimit ?? -1), isDefault: plan.isDefault } }
...
export function SubscriptionPlanManagement({ plans, mode, planId, onChanged }: Props) {
  ...
  if (mode === 'list') return <section>
    ...
    {loading ? <PlanSkeleton/> : adminPlans.length === 0 ? <Link href="/admin/plans/new" className="block w-full rounded-[22px] border border-dashed border-slate-300 bg-white p-12 text-center text-sm text-slate-500">还没有订阅计划，创建第一个订阅计划</Link> : visiblePlans.length === 0 ? <p className="rounded-[18px] bg-white px-5 py-12 text-center text-sm text-slate-400">没有匹配的订阅计划</p> : <div className="space-y-3">{visiblePlans.map((plan) => {
      ...
      return <Link key={plan.id} href={`/admin/plans/${plan.id}`} className="grid w-full grid-cols-[1fr_auto] gap-5 rounded-[18px] bg-white p-5 text-left transition hover:-translate-y-0.5 hover:shadow-[0_14px_34px_rgba(39,55,92,.07)] active:translate-y-px"><div className="min-w-0"><div className="flex items-center gap-2"><h3 className="truncate font-semibold">{plan.name}</h3>{plan.isDefault && <span className="rounded-md bg-[#edf2ff] px-2 py-0.5 text-[10px] font-medium text-[#3157d5]">默认</span>}</div><p className="mt-1 truncate text-xs text-slate-400">{plan.description || '暂无描述'}</p><div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-xs text-slate-500"><span className="flex items-center gap-1.5"><Gauge size={13}/>{quotaText(plan.rateLimit)} 次/分钟</span><span>{quotaText(plan.dailyLimit)} / 日</span><span>{quotaText(plan.weeklyLimit)} / 周</span><span>{quotaText(plan.monthlyLimit)} / 月</span></div></div><div className="text-right"><p className="text-2xl font-semibold tabular-nums">{subscriberCount.toLocaleString()}</p><p className="mt-1 text-[11px] text-slate-400">有效订阅</p></div></Link>
    })}</div>}
  </section>
