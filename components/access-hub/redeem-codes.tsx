...
export function RedeemCodes({ authenticated, isAdmin, skus, onChanged, onSignIn, mode }: Props) {
  ...
  const [generator, setGenerator] = useState({ skuId: skus[0]?.id || '', count: '10' })
  ...
  const activeSkus = (adminData?.skus ?? skus).filter((sku) => sku.active)
