...
import type { AdminData, ApiService, ServiceApi, ServiceParameter } from './types'
import { useWorkspaceRefresh } from './workspace-data'
import { requestJson } from '@/lib/http-client'

type Mode = 'list' | 'new' | 'edit' | 'api-new' | 'api-edit'
type ServiceForm = { code: string; name: string; description: string; baseUrl: string; authType: ApiService['authType']; authToken: string; authHeader: string; authValue: string; authUsername: string; authPassword: string; enabled: boolean }
...
const emptyService: ServiceForm = { code: '', name: '', description: '', baseUrl: '', authType: 'none', authToken: '', authHeader: 'x-api-key', authValue: '', authUsername: '', authPassword: '', enabled: true }
...
function serviceFormFor(service: ApiService): ServiceForm { return { ...emptyService, code: service.code, name: service.name, description: service.description, baseUrl: service.baseUrl, authType: service.authType, enabled: service.enabled } }
...
export function Services({ mode, resourceId }: { mode: Mode; resourceId?: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try { const data = await requestJson<AdminData>('/api/admin?section=services', { cache: 'no-store' }); setServices(data.services ?? []); setNotice(null) }
  ...
  const visible = services.filter((service) => !query || `${service.code} ${service.name} ${service.description} ${service.baseUrl}`.toLowerCase().includes(query.toLowerCase()))
  ...
  if (mode === 'new' || mode === 'edit') return <ServiceEditor mode={mode} service={selectedService} form={serviceForm} setForm={setServiceForm} dirty={serviceDirty} saving={saving} notice={notice} onReset={() => setServiceForm(serviceBaseline)} onSave={saveService}/>
...
function ServiceEditor({ mode, service, form, setForm, dirty, saving, notice, onReset, onSave }: { mode: 'new' | 'edit'; service: ApiService | null; form: ServiceForm; setForm: (value: ServiceForm) => void; dirty: boolean; saving: boolean; notice: { tone: 'success' | 'error'; text: string } | null; onReset: () => void; onSave: () => Promise<void> }) {
  return <div className="mx-auto max-w-4xl"><Link href="/admin/services" className="mb-5 inline-flex items-center gap-2 text-sm text-slate-500 hover:text-[#3157d5]"><ArrowLeft size={15}/>返回服务列表</Link><form onSubmit={(event) => { event.preventDefault(); void onSave() }} className="rounded-[24px] bg-white p-6 shadow-[0_18px_55px_rgba(39,55,92,.06)] sm:p-8"><EditorHeading eyebrow={mode === 'new' ? '新服务' : '服务连接'} title={mode === 'new' ? '连接真实 API 服务' : service?.name || ''} dirty={dirty}/><div className="mt-8 grid gap-5 sm:grid-cols-2"><Field label="服务编码" hint="用于用户调用路径，保存后仍可修改"><input className="access-input font-mono lowercase" value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value.toLowerCase() })} placeholder="example-ai"/></Field><Field label="服务名称"><input className="access-input" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="示例 AI 服务"/></Field><div className="sm:col-span-2"><Field label="Base URL" hint="生产环境仅允许 HTTPS，且不能指向私有网络"><input className="access-input font-mono" value={form.baseUrl} onChange={(event) => setForm({ ...form, baseUrl: event.target.value })} placeholder="https://api.example.com/v1"/></Field></div><div className="sm:col-span-2"><Field label="说明"><textarea rows={3} className="access-input resize-none" value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} placeholder="说明此服务的能力和适用场景"/></Field></div></div>
      <section className="mt-7 rounded-2xl border border-slate-200 p-5"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-[#edf2ff] text-[#3157d5]"><ShieldCheck size={16}/></span><div><h3 className="text-sm font-medium">上游鉴权</h3><p className="mt-0.5 text-xs text-slate-400">密钥加密保存，保存后不会在页面或 API 中回显。</p></div></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="鉴权方式"><select className="access-input" value={form.authType} onChange={(event) => setForm({ ...form, authType: event.target.value as ApiService['authType'] })}><option value="none">无需鉴权</option><option value="bearer">Bearer Token</option><option value="header">自定义 Header</option><option value="basic">Basic Auth</option></select></Field>{form.authType === 'bearer' && <Field label="Token" hint={service?.authConfigured ? '留空则保留现有 Token' : undefined}><input type="password" autoComplete="new-password" className="access-input" value={form.authToken} onChange={(event) => setForm({ ...form, authToken: event.target.value })} placeholder={service?.authConfigured ? '已安全配置' : '输入上游 Token'}/></Field>}{form.authType === 'header' && <><Field label="Header 名称"><input className="access-input font-mono" value={form.authHeader} onChange={(event) => setForm({ ...form, authHeader: event.target.value })} placeholder="x-api-key"/></Field><Field label="Header 值" hint={service?.authConfigured ? '留空则保留现有值' : undefined}><input type="password" autoComplete="new-password" className="access-input" value={form.authValue} onChange={(event) => setForm({ ...form, authValue: event.target.value })} placeholder={service?.authConfigured ? '已安全配置' : '输入密钥'}/></Field></>}{form.authType === 'basic' && <><Field label="用户名"><input className="access-input" value={form.authUsername} onChange={(event) => setForm({ ...form, authUsername: event.target.value })}/></Field><Field label="密码" hint={service?.authConfigured ? '如需更换，请同时重新填写用户名和密码' : undefined}><input type="password" autoComplete="new-password" className="access-input" value={form.authPassword} onChange={(event) => setForm({ ...form, authPassword: event.target.value })}/></Field></>}</div></section>
...
function ApiEditor({ mode, service, form, setForm, dirty, saving, notice, onReset, onSave }: { mode: 'api-new' | 'api-edit'; service: ApiService; form: ApiForm; setForm: (value: ApiForm) => void; dirty: boolean; saving: boolean; notice: { tone: 'success' | 'error'; text: string } | null; onReset: () => void; onSave: () => Promise<void> }) {
  ...
  return <div className="mx-auto max-w-4xl"><Link href={`/admin/services/${service.id}`} className="mb-5 inline-flex items-center gap-2 text-sm text-slate-500 hover:text-[#3157d5]"><ArrowLeft size={15}/>返回 {service.name}</Link><form onSubmit={(event) => { event.preventDefault(); void onSave() }} className="rounded-[24px] bg-white p-6 shadow-[0_18px_55px_rgba(39,55,92,.06)] sm:p-8"><EditorHeading eyebrow={`${service.code} / API`} title={mode === 'api-new' ? '定义用户调用端点' : form.name} dirty={dirty}/><div className="mt-8 grid gap-5 sm:grid-cols-2"><Field label="API 编码" hint="组成公开网关路径"><input className="access-input font-mono lowercase" value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value.toLowerCase() })} placeholder="chat"/></Field><Field label="API 名称"><input className="access-input" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="对话生成"/></Field><Field label="请求方式"><select className="access-input" value={form.method} onChange={(event) => setForm({ ...form, method: event.target.value as ServiceApi['method'] })}>{['GET', 'POST', 'PUT', 'PATCH', 'DELETE'].map((method) => <option key={method}>{method}</option>)}</select></Field><Field label="上游路径" hint="路径参数使用 {id} 占位"><input className="access-input font-mono" value={form.path} onChange={(event) => setForm({ ...form, path: event.target.value })} placeholder="/chat/completions"/></Field><Field label="单次计费次数" hint="一次成功进入网关的请求消耗多少配额"><input type="number" min="1" max="10000" className="access-input tabular-nums" value={form.usageUnits} onChange={(event) => setForm({ ...form, usageUnits: event.target.value })}/></Field><Field label="上游超时（毫秒）"><input type="number" min="1000" max="120000" step="1000" className="access-input tabular-nums" value={form.timeoutMs} onChange={(event) => setForm({ ...form, timeoutMs: event.target.value })}/></Field><div className="sm:col-span-2"><Field label="说明"><textarea rows={3} className="access-input resize-none" value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })}/></Field></div></div>
      ...
      <div className="mt-5 rounded-xl bg-blue-50/70 px-4 py-3 text-xs leading-5 text-blue-700"><strong>公开调用路径</strong><code className="ml-2">{form.method} /api/gateway/{service.code}/{form.code || '{api}'}</code><span className="mt-1 block text-blue-600/80">请求进入网关后先预留 {form.usageUnits || '1'} 次配额，再调用上游；上游超时或失败也会计费，防止未知结果被重复调用。</span></div><Toggle checked={form.enabled} onChange={(enabled) => setForm({ ...form, enabled })} title="启用 API" detail="停用后网关返回 404，不再接受新的用户调用。"/>{notice && <div className="mt-5"><Notice value={notice}/></div>}<EditorActions dirty={dirty} saving={saving} onReset={onReset} label={mode === 'api-new' ? '创建 API' : '保存 API'}/>
    </form></div>
}
...
function authLabel(type: ApiService['authType']) { return ({ none: '无鉴权', bearer: 'Bearer', header: 'Header', basic: 'Basic Auth' })[type] }
