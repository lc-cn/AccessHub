...
export function Overview({ dashboard, loading, copied, onCopyId, onNavigate }: Props) {
  ...
  return <div className="space-y-7">
    ...
    <section><div className="mb-4"><p className="text-xs font-medium tracking-wide text-[#3157d5]">权益阶梯</p><h3 className="mt-1 text-lg font-semibold">各订阅计划配额</h3><p className="mt-1 text-xs text-slate-400">对比访问能力，选择适合当前调用规模的权益。</p></div><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{dashboard?.plans.map((item) => <PlanTier key={item.id} plan={item} current={item.id === plan?.planId} onRedeem={() => onNavigate('兑换码')}/>)}</div></section>
...
function PlanTier({ plan, current, onRedeem }: { plan: DashboardPlan; current: boolean; onRedeem: () => void }) { return <article className={`rounded-[18px] p-5 ${current ? 'bg-[#edf2ff] ring-1 ring-[#bdcaff]' : 'bg-white'}`}><div className="flex items-start justify-between"><div><h4 className="font-semibold">{plan.name}</h4><p className="mt-1 line-clamp-2 text-xs leading-5 text-slate-400">{plan.description || '适用于不同规模的 API 调用。'}</p></div>{current && <span className="rounded-md bg-[#3157d5] px-2 py-1 text-[10px] font-medium text-white">当前</span>}</div><dl className="mt-5 grid grid-cols-2 gap-3 text-xs"><Quota label="分钟" value={plan.rateLimit}/><Quota label="每日" value={plan.dailyLimit}/><Quota label="每周" value={plan.weeklyLimit}/><Quota label="每月" value={plan.monthlyLimit}/></dl>{!current && (plan.purchaseUrl ? <a href={plan.purchaseUrl} target="_blank" rel="noreferrer" className="mt-5 flex items-center gap-1.5 text-xs font-medium text-[#3157d5] transition hover:gap-2.5">立即购买此权益 <ExternalLink size={13}/></a> : <button onClick={onRedeem} className="mt-5 flex items-center gap-1.5 text-xs font-medium text-slate-500 transition hover:text-[#3157d5]">使用兑换码 <ArrowRight size={13}/></button>)}</article> }
