...
import { Activity, ArrowRight, Check, Clock3, Coins, Copy, ExternalLink, Settings2, Ticket, Users } from 'lucide-react'
...
export function Overview({ dashboard, loading, copied, onCopyId, onNavigate }: Props) {
  ...
  return <div className="space-y-7">
    ...
    <section className="rounded-[22px] bg-white p-6 shadow-[0_18px_55px_rgba(39,55,92,.06)] sm:p-7">
      <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-medium tracking-wide text-slate-400">账户身份</p><h3 className="mt-1 text-lg font-semibold">调用凭据归属</h3></div><Link href="/account" className="flex items-center gap-1.5 rounded-lg bg-slate-100 px-2.5 py-1.5 text-[11px] font-medium text-slate-500 transition hover:bg-[#edf2ff] hover:text-[#3157d5]"><Settings2 size={12}/>管理账户</Link></div>
...
function PlanTier({ plan, action, onRedeem }: { plan: DashboardPlan; action: PlanAction; onRedeem: () => void }) {
  ...
  const label = { included: '已包含', current: '当前计划', downgrade: '低阶计划', upgrade: '升级' }[action]
  return <article className={`rounded-[18px] p-5 ${action === 'current' ? 'bg-[#edf2ff] ring-1 ring-[#bdcaff]' : 'bg-white'}`}><div className="flex items-start justify-between"><div><h4 className="font-semibold">{plan.name}</h4><p className="mt-1 line-clamp-2 text-xs leading-5 text-slate-400">{plan.description || '适用于不同规模的 API 调用。'}</p></div><span className={`rounded-md px-2 py-1 text-[10px] font-medium ${action === 'current' ? 'bg-[#3157d5] text-white' : 'bg-slate-100 text-slate-500'}`}>{label}</span></div><dl className="mt-5 grid grid-cols-2 gap-3 text-xs"><Quota label="分钟" value={plan.rateLimit}/><Quota label="每日" value={plan.dailyLimit}/><Quota label="每周" value={plan.weeklyLimit}/><Quota label="每月" value={plan.monthlyLimit}/></dl>{active ? (plan.purchaseUrl ? <a href={plan.purchaseUrl} target="_blank" rel="noreferrer" className="mt-5 flex items-center gap-1.5 text-xs font-medium text-[#3157d5] transition hover:gap-2.5">购买并升级 <ExternalLink size={13}/></a> : <button onClick={onRedeem} className="mt-5 flex items-center gap-1.5 text-xs font-medium text-[#3157d5] transition hover:gap-2.5">使用兑换码升级 <ArrowRight size={13}/></button>) : <p className="mt-5 text-xs font-medium text-slate-300">{action === 'downgrade' ? '当前计划已包含更多额度' : label}</p>}</article>
}
