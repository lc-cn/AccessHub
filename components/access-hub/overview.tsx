import { Activity, ArrowRight, Check, Clock3, Copy, ExternalLink, Ticket, Users } from 'lucide-react'
  ...
  return <div className="space-y-7">
    ...
    <section className="grid gap-6 xl:grid-cols-[1.25fr_.75fr]">
      <article className="rounded-[22px] bg-white p-6 shadow-[0_18px_55px_rgba(39,55,92,.06)] sm:p-7">
        ...
        <div className="mt-7 flex items-center justify-between rounded-xl bg-[#f5f7fb] px-4 py-3"><span><span className="block text-xs font-medium text-slate-600">瞬时访问频率</span><span className="mt-0.5 block text-[11px] text-slate-400">滚动 60 秒窗口</span></span><strong className="font-mono text-sm text-[#3157d5]">{group?.rateLimit?.toLocaleString() ?? '—'} 次/分钟</strong></div>
      </article>
...
function UsageBar({ label, used, limit }: { label: string; used: number; limit: number | null | undefined }) {
  ...
  return <div><div className="mb-2 flex items-baseline justify-between"><span className="text-xs font-medium text-slate-600">{label}</span><span className="font-mono text-xs text-slate-400"><strong className="font-semibold text-slate-700">{used.toLocaleString()}</strong> / {limit?.toLocaleString() ?? '不限额'}</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full transition-[width] duration-500 ${percentage >= 90 ? 'bg-rose-500' : percentage >= 70 ? 'bg-amber-500' : 'bg-[#3157d5]'}`} style={{ width: limit ? `${percentage}%` : '4%' }}/></div></div>
}
...
function Quota({ label, value }: { label: string; value: number | null }) { return <div><dt className="text-[10px] text-slate-400">{label}</dt><dd className="mt-0.5 font-mono font-medium text-slate-600">{value?.toLocaleString() ?? '不限'}</dd></div> }
function limitLabel(limit: number | null | undefined, period: string) { return limit == null ? `${period}不限额` : `${period}上限 ${limit.toLocaleString()}` }
