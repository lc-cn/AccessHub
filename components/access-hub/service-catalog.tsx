...
function TestConsole({ api }: { api: CatalogApi }) {
  ...
  return <aside className="rounded-[24px] bg-[#182238] p-6 text-white xl:sticky xl:top-28 xl:self-start"><div className="flex items-start justify-between"><span className="grid size-10 place-items-center rounded-xl bg-white/10"><FlaskConical size={18}/></span><span className="rounded-lg bg-amber-400/15 px-2.5 py-1.5 text-[10px] text-amber-200">测试也会计费</span></div><h3 className="mt-5 text-lg font-semibold">测试调用</h3><p className="mt-1 text-xs leading-5 text-slate-400">真实请求会发送到上游，并消耗 {api.usageUnits} 次配额。上游失败或超时也会计费。</p><div className="mt-5 space-y-3">{api.parameters.length === 0 ? <p className="rounded-xl bg-white/[0.06] px-4 py-4 text-xs text-slate-400">此 API 无需输入参数</p> : api.parameters.map((parameter) => <label key={parameterKey(parameter)} className="block"><span className="mb-1.5 flex items-center justify-between text-[11px] text-slate-300"><span>{parameter.name}{parameter.required && <span className="ml-1 text-rose-300">*</span>}</span><span className="font-mono text-[9px] text-slate-500">{parameter.location} · {parameter.dataType}</span></span>{parameter.dataType === 'json' ? <textarea rows={4} value={values[parameterKey(parameter)] || ''} onChange={(event) => setValues({ ...values, [parameterKey(parameter)]: event.target.value })} className="w-full resize-y rounded-xl border border-white/10 bg-white/[0.07] px-3 py-2.5 font-mono text-xs text-white outline-none focus:border-[#7893ee]" placeholder={parameter.description || '{ "key": "value" }'}/> : <input value={values[parameterKey(parameter)] || ''} onChange={(event) => setValues({ ...values, [parameterKey(parameter)]: event.target.value })} className="h-10 w-full rounded-xl border border-white/10 bg-white/[0.07] px-3 text-xs text-white outline-none focus:border-[#7893ee]" placeholder={parameter.dataType === 'boolean' ? 'true 或 false' : parameter.description}/>}</label>)}</div>{error && <p className="mt-4 rounded-xl bg-rose-400/10 px-3 py-2.5 text-xs text-rose-200">{error}</p>}<button onClick={() => void run()} disabled={running} className="mt-5 flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-white text-sm font-semibold text-[#182238] transition hover:bg-slate-100 disabled:opacity-60"><FlaskConical size={15}/>{running ? '请求中…' : `测试并消耗 ${api.usageUnits} 次配额`}</button>{result && <div className="mt-5 overflow-hidden rounded-xl bg-black/20"><div className="flex flex-wrap items-center gap-3 border-b border-white/10 px-3 py-2 text-[10px]"><span className={result.status >= 200 && result.status < 300 ? 'text-emerald-300' : 'text-rose-300'}>{result.status} {result.statusText}</span><span className="flex items-center gap-1 text-slate-400"><Clock3 size={10}/>{result.durationMs}ms</span>{result.usageUnits && <span className="flex items-center gap-1 text-amber-200"><Gauge size={10}/>{result.usageUnits} 次</span>}</div><pre className="max-h-80 overflow-auto p-3 text-[11px] leading-5 text-slate-300"><code>{result.body}</code></pre></div>}</aside>
}
