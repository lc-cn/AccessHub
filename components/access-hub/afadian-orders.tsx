...
export function AfdianOrders({ mode, orderId }: { mode: 'list' | 'detail'; orderId?: string }) {
  ...
  const visible = orders.filter((order) => !query || `${order.outTradeNo} ${order.afdianPlanTitle} ${order.afdianPlanId} ${order.codes.map((code) => code.code).join(' ')}`.toLowerCase().includes(query.toLowerCase()))
