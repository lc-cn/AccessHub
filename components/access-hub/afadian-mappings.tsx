...
export function AfdianMappings({ groups, mode, ruleId }: Props) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try { const result = await requestJson<AdminData>('/api/admin?section=afdian', { cache: 'no-store' }); setRules(result.afdianRules ?? []); setConfigured(Boolean(result.afadianWebhookConfigured)) }
